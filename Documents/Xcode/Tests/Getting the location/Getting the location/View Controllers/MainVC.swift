        ...
        DispatchQueue.global().async {
            ...
            if let url = URL(string: urlString) {
                if let data = try? Data(contentsOf: url) {
                    ...
                    let weatherTempMin = result["main"]["temp_min"].stringValue
                    let weatherTempMax = result["main"]["temp_max"].stringValue
                    let weatherDesc = result["weather"][0]["description"].stringValue
                    let cityName = result["name"].stringValue
                    
                    
                    let weatherTempDouble = Double(weatherTemp)
                    let weatherTempMinDouble = Double(weatherTempMin)
                    let weatherTempMaxDouble = Double(weatherTempMax)
